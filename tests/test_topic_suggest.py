...
def test_validate_topic_suggestion():
    ok, err = dr.validate_topic_suggestion({"suggested_by": "ext:u1", "pro": "该", "con": "不该", "title": "要不要", "tags": ["伦理"]})
...
def test_suggest_endpoint_dedupes_and_flags_known(tmp_path, monkeypatch):
    ...
    r = c.post("/api/debate/topics/suggest", json={"suggested_by": "ext:u1", "pro": "新正", "con": "新反", "title": "新题"})
    assert r.status_code == 200 and r.json()["status"] == "pending" and r.json()["already_in_bank"] is False
    sid = r.json()["id"]
    # 同人同题：不重复落
    r2 = c.post("/api/debate/topics/suggest", json={"suggested_by": "ext:u1", "pro": "新正", "con": "新反"})
    ...
    r4 = c.post("/api/debate/topics/suggest", json={"suggested_by": "ext:u2", "pro": "已有正", "con": "已有反"})
